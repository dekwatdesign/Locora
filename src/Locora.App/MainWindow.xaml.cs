...
public sealed partial class MainWindow : Window
{
    ...
    private IEnumerable<CommandPaletteItem> CreateCommandPaletteItems()
    {
        ...
        foreach (var profile in ViewModel.StackProfiles.Where(profile => profile.CanSelect))
        {
            ...
            yield return new CommandPaletteItem(
                $"Select Stack Profile: {profile.DisplayName}",
                profile.Summary,
                $"stack profile select active services packages start all {profileSearchText}",
                () => ExecuteCommandAsync(profile.SelectProfileCommand, profile));
        }

        foreach (var project in ViewModel.Projects)
        {
            var projectSearchText = $"{project.Name} {project.Url} {project.Runtime} {project.Description} {project.TagsLabel} {project.Folder}";
